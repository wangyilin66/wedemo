...
BM.Draw.Polyline = BM.Draw.Feature.extend({
	...
	_onMouseMove: function (e) {
		...
		BM.DomEvent.preventDefault(e.originalEvent);
	},
	...
	_startPoint: function (clientX, clientY) {
		this._mouseDownOrigin = BM.point(clientX, clientY);
	},
	...
	_endPoint: function (clientX, clientY, e) {
		if (this._mouseDownOrigin) {
			var dragCheckDistance = BM.point(clientX, clientY)
				.distanceTo(this._mouseDownOrigin);
			var lastPtDistance = this._calculateFinishDistance(e.latlng);
			if (this.options.maxPoints > 1 && this.options.maxPoints == this._markers.length + 1) {
				this.addVertex(e.latlng);
				this._finishShape();
			} else if (lastPtDistance < 10 && BM.Browser.touch) {
				this._finishShape();
	...
	_calculateFinishDistance: function (potentialLatLng) {
		...
		if (this._markers.length > 0) {
			...
			} else if (this.type === BM.Draw.Polygon.TYPE) {
				finishMarker = this._markers[0];
			} else {
				return Infinity;
			}
			var lastMarkerPoint = this._map.latLngToContainerPoint(finishMarker.getLatLng()),
				potentialMarker = new BM.Marker(potentialLatLng, {
	...
	_createMarker: function (latlng) {
		var marker = new BM.Marker(latlng, {
			icon: this.options.icon,
			zIndexOffset: this.options.zIndexOffset * 2
		});
	...
	_drawGuide: function (pointA, pointB) {
		...
		if (!this._guidesContainer) {
			this._guidesContainer = BM.DomUtil.create('div', 'bigemap-draw-guides', this._overlayPane);
		}
		...
		for (; i < length; i += this.options.guidelineDistance) {
			...
			dash = BM.DomUtil.create('div', 'bigemap-draw-guide-dash', this._guidesContainer);
			dash.style.backgroundColor =
				!this._errorShown ? this.options.shapeOptions.color : this.options.drawError.color;

			BM.DomUtil.setPosition(dash, dashPoint);
		}
	},
	...
	_getMeasurementString: function () {
		...
		return BM.GeometryUtil.readableDistance(distance, this.options.metric, this.options.feet, this.options.nautic, this.options.precision);
	},
	...
	_fireCreatedEvent: function () {
		...
		BM.Draw.Feature.prototype._fireCreatedEvent.call(this, poly);
	}
});
