...
import { Drafty } from '../tinodesdk/tinode';
...
class TinodeWeb extends React.Component {
  ...
  getBlankState() {
    ...
    return {
      ...
      serverAddress: '47.94.235.90:6060',//settings.serverAddress || detectServerAddress(),
  ...
  componentDidMount() {
    ...
    const token = LocalStorageUtil.getObject('auth-token') ;
  ...
  componentWillReceiveProps(nextProps) {  
    if (nextProps.Imflag!== this.props.Imflag) {
      console.log("next:" + nextProps.Imflag+"prev : " +this.props.Imflag);
      switch(nextProps.Imflag){
        case "通讯录":
        this.handleNewTopic();
        break;
        case "聊天记录":
      ...
      // if(nextProps)
      // let plist = nextProps.poslist;
      // this.handleSendPosition(JSON.stringify(plist));
    }


      this.handleTopicSelected(nextProps.userId.userid);
      console.log("接收到userID" + nextProps.userId.userid)


  }
  // static getDerivedStateFromProps(nextProps, prevState) {

  //   if (nextProps.Imflag !== prevState.Imflag) {
  //     console.log("next:" + nextProps.Imflag+"prev : " +prevState);
  ...
  tnMeMetaDesc(desc) {
    if (desc && desc.public) {
      this.props.sendUserInfo(  {
        userID:this.tinode.getCurrentUserID(),
        userNick:desc.public.fn
      })
  ...
  resetContactList() {
    ...
    //聊天列表 注入 redux
    this.props.sendTopicList(newState.chatList)
  ...
  handleTopicSelected(topicName, unused_index, online, acs) {
    console.log('displayMobile : ' + this.state.displayMobile);
  ...
  handleHideMessagesView() {
   ...
   handleSendPosition(e){
       this.handleSendMessage(
       Drafty.insertPosition
       (null,0,39.926588421909436,116.5364396804984));
   }
...
//从通讯录切换到聊天列表
  ...
  render() {
   ...
   <SidepanelView
          ...
          onOnlineChange={this.handleTopicSelectedOnline.bind(this)}
          onError={this.handleError}
...
export default injectIntl(connect(mapStateToProps,mapDispatchToProps)(TinodeWeb));
